/**
 * Turn text and teacher marks into tremored polylines.
 *
 * Lifted from the owner's prototype on branch
 * `origin/cursor/realtime-math-tutor-engine-5707`, file `src/lib/tutor/hand/compose.ts`.
 * Changes while lifting: strict types, one segment per glyph subpath (see `path.ts`),
 * the alternate picker exported so `mathLayout.ts` can share it, and unused exports
 * dropped.
 */
...
/** One pen-down..pen-up stroke, ready to become a tldraw draw segment. */
export type InkSegment = {
  type: "free" | "straight";
  points: InkPt[];
};

/** A composed run of ink in local coordinates (origin top-left). */
...
/** Default hand size in px: the atlas em box is 14 tall, so this is 1:1. */
export const HAND_SIZE = 14;
/** Live notes: large enough to read next to student ink. */
export const NOTE_HAND_SIZE = 22;
/** Baseline-to-baseline distance for prose, in em units. */
export const LINE_HEIGHT = 18;

/** Simon lock: 4px advance between glyphs. */
const ADVANCE_PX = 4;
const WORD_GAP_PX = ADVANCE_PX * 2;
/** Never sit on the ruled line. */
const JITTER_MIN = 0.4;
const JITTER_SPAN = 0.85;

/** Which alternate of which glyph was stamped last, so we never repeat one. */
export type GlyphPick = { key: string; alt: number };

function baselineJitter(rng: () => number): number {
  const sign = rng() < 0.5 ? -1 : 1;
  return sign * (JITTER_MIN + rng() * JITTER_SPAN);
}

/**
 * Pick an alternate for `ch`, never the same alternate twice in a row.
 * Mutates `prev` so callers can thread it through a whole line.
 */
export function pickGlyph(ch: string, rng: () => number, prev: GlyphPick): Glyph | null {
  ...
  for (let w = 0; w < words.length; w++) {
    ...
    for (const ch of tokenizeHand(word)) {
      ...
      for (const path of glyph.paths) {
        for (const pts of samplePath(path)) {
...
/** Teacher mark: a loose circle round something worth a second look. */
...
/** Teacher mark: "something goes here". */
...
/** Teacher mark: underline the bit that matters. */
...
/** Geometry: angle arc in the same dry nib — not letters. */
...
/** Teacher mark: "this becomes that". */
